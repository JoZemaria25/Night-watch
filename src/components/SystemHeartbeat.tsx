    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                if (!user) {
                    console.warn("[SystemHeartbeat] No authenticated user.");
                    setLoading(false);
                    return;
                }
                ...
                if (profileError || !profile?.organization_id) {
                    console.warn("[SystemHeartbeat] No organization linked to user.");
                ...
                console.error("[SystemHeartbeat] Unexpected error:", err);
            } finally {
                setLoading(false);
            }
        }
            ...
            <div className="mb-6 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-zinc-400">Night Watch Engine Activity</p>
                    </div>
                </div>
                <div className="text-right">
                    <p className="text-2xl font-bold text-white">{loading ? "-" : totalEvents}</p>
                    <p className="text-xs text-zinc-500 uppercase tracking-wider">Events (7d)</p>
                </div>
            </div>
